...
插件会自动为每个动态集合的 `_id` 字段创建索引，并在数据超过上限时删除最旧的记录，始终保持最新的 8,400 条滚动窗口数据。

**工作原理**：
- 当首次向某个集合写入数据时，自动在该集合的 `_id` 字段上创建升序索引
- 每次插入新数据后，检查集合文档总数
- 当文档数超过 8,400 条时，通过 MongoDB `find` 命令指定 `sort: {_id: 1}` 和 `limit: N`，仅取最旧 N 条文档的 `_id`，无需全量拉取或内存排序
- 使用 `$in` 批量删除，一次操作清除所有旧文档
...
- 如果索引创建失败，插件会记录 debug 日志，不影响数据写入
- 手动在 MongoDB 中查看索引：`db.集合名.getIndexes()`
- 仅对 `insert` 操作触发裁剪，`upsert`（更新）不会触发
